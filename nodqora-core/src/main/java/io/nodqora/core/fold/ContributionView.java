 ...
 *
 * <p>{@code health} is {@code UNKNOWN} only alongside non-empty {@code metrics}: an empty abstention
 * is an omission and is dropped before it reaches the store (ADR-0104, ADR-0165). An empty one is
 * not forbidden by this type, because the fold enforces the same rule itself rather than relying on
 * its caller — a pure function that is only correct given a well-formed input is a trap for the next
 * reader.
 */
