 ...
 * emits no edges, and the claim is about all of them.
 */
export function edgelessSentence(environmentDisplayName: string): string {
  return `The plugins reading ${environmentDisplayName} report nodes, not what connects them. Edges come from a topology file.`
}
